...
passwd_entry_t* parse_passwd(struct options* options, int* nr_entries)
{
	...
	if ((passwd=fopen(options->route,"r"))==NULL) {
		fprintf(stderr, "%s could not be opened: ", options->route);
		perror(NULL);
		return NULL;
	}
	...
	while (fgets(line, MAX_PASSWD_LINE + 1, passwd) != NULL) {
		...
		while((token = strsep(&lineptr, options->separator))!=NULL) {
			switch(token_id) {
			case LOGIN_NAME_IDX:
